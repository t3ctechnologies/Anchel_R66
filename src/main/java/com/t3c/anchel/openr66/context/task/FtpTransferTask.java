...
public class FtpTransferTask extends AbstractTask {
    ...
    public void run() {
        ...
        WaarpFtpClient ftpClient =
				new WaarpFtpClient(requested, port, user, pwd, acct, isPassive, ssl,
        ...
        if (!status) {
            ...
            futureCompletion.setResult(result);
            futureCompletion.setFailure(exception);
            logger.error("Cannot connect to remote FTP host");
            return;
        }
        try {
            ...
            if (digest > 0) {
                ...
                if (hashresult == null) {
                    ...
                    futureCompletion.setResult(result);
                    futureCompletion.setFailure(exception);
                    logger.error("Hash cannot be computed: " );
                    return;
                }
                // now check locally
                String hash;
