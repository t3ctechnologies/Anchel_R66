...
public class DirectTransfer extends AbstractTransfer {
	...
	public DirectTransfer(R66Future future, String remoteHost, String filename, String rulename, String fileinfo,
			boolean isMD5, int blocksize, long id, NetworkTransaction networkTransaction) {
		// no starttime since it is direct (blocking request, no delay)
		super(DirectTransfer.class, future, filename, rulename, fileinfo, isMD5, remoteHost, blocksize, id, null);
	...
	public void run() {
		...
		for (int i = 0; i < Configuration.RETRYNB; i++) {
			try {
				...
				future.setResult(new R66Result(e, null, true, ErrorCode.Internal, taskRunner));
				future.setFailure(e);
				return;
			} catch (OpenR66ProtocolNoConnectionException e) {
				logger.debug("Cannot Connect", e);
				future.setResult(new R66Result(e, null, true, ErrorCode.ConnectionImpossible, taskRunner));
				...
				future.setResult(new R66Result(e, null, true, ErrorCode.TransferError, taskRunner));
				future.setFailure(e);
				return;
		...
		if (exc != null) {
			...
			future.setResult(new R66Result(exc, null, true, ErrorCode.ConnectionImpossible, taskRunner));
			// since no connection : just forget it
			if (nolog || taskRunner.shallIgnoreSave()) {
				try {
					taskRunner.delete();
	...
	public static void main(String[] args) {
		...
		try {
			DirectTransfer transaction = new DirectTransfer(future, rhost, localFilename, rule, fileInfo, ismd5, block,
			...
			if (future.isSuccess()) {
				...
				outputFormat.setValue("filefinal",
				...
				if (nolog || result.getRunner().shallIgnoreSave()) {
					...
					try {
						...
						logger.warn("Cannot apply nolog to     " + result.getRunner().toShortString(), e);
					}
				}
