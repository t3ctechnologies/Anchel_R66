 ...
 * changed the valuesin the database but only dynamic values while the server is
 * running and until it is shutdown.
 * 
...
public class ChangeBandwidthLimits implements Runnable {
	...
	public void run() {
		...
		try {
			...
			future.setResult(new R66Result(
					new OpenR66ProtocolNoConnectionException("Cannot connect to server " + host.getHostid()), null,
	...
	public static void main(String[] args) {
		...
		try {
			ChangeBandwidthLimits transaction = new ChangeBandwidthLimits(future, swriteGlobalLimit, sreadGlobalLimit,
					swriteSessionLimit, sreadSessionLimit, networkTransaction);
			if (stohost != null) {
				try {
			...
			if (future.isSuccess()) {
				...
				if (result.getCode() == ErrorCode.Warning) {
					logger.warn("WARNED on bandwidth:     " + sresult + "     delay: " + delay);
				} else {
					logger.warn("SUCCESS on Bandwidth:     " + sresult + "     delay: " + delay);
				}
			} else {
				if (result.getCode() == ErrorCode.Warning) {
