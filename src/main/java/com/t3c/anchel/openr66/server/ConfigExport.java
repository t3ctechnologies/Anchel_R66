...
public class ConfigExport implements Runnable {
	...
	public ConfigExport(R66Future future, boolean host, boolean rule, boolean business, boolean alias, boolean role,
	 ...
	 * been initialized. It is the responsibility of the caller to finish all
	 * network resources.
	 */
	public void run() {
		if (logger == null) {
		...
		if (!(host || rule || business || alias || role)) {
			...
			future.setResult(new R66Result(new OpenR66ProtocolNoDataException("No action required"), null, true,
		...
		try {
			...
			future.setResult(new R66Result(
					new OpenR66ProtocolNoConnectionException("Cannot connect to server " + dbhost.getHostid()), null,
		...
		LocalChannelReference localChannelReference = networkTransaction.createConnectionWithRetry(socketAddress, isSSL,
				future);
		socketAddress = null;
	...
	protected static boolean getParams(String[] args) {
		...
		if (!FileBasedConfiguration.setClientConfigurationFromXml(Configuration.configuration, args[0])) {
			logger.error(_INFO_ARGS);
			return false;
		}
	...
	public static void main(String[] args) {
		...
		try {
			ConfigExport transaction = new ConfigExport(future, shost, srule, sbusiness, salias, srole,
					networkTransaction);
			if (stohost != null) {
				try {
			...
			if (future.isSuccess()) {
				...
				if (useJson) {
					...
					message = (result.getOther() != null ? ((ValidPacket) result.getOther()).getSheader() : "no file");
				}
				if (result.getCode() == ErrorCode.Warning) {
					logger.warn("WARNED on files:     " + message + "     delay: " + delay);
				} else {
					logger.warn("SUCCESS on Final files:     " + message + "     delay: " + delay);
				}
			} else {
				if (result.getCode() == ErrorCode.Warning) {
